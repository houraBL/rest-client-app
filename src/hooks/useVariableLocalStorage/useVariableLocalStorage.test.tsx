...
describe('useVariableLocalStorage', () => {
  const key = 'test-key';
  const TEST_VALUES = {
    default: 'default',
    initial: 'initial',
  ...
  it('Initializes with default value if localStorage is empty', () => {
    const { result } = renderHook(() =>
      useVariableLocalStorage(key, TEST_VALUES.default)
    );

    const [storedValue] = result.current;
    expect(storedValue).toBe(TEST_VALUES.default);
    expect(localStorage.getItem(key)).toBe(JSON.stringify(TEST_VALUES.default));
  });

  it('Initializes with stored value if localStorage has data', () => {
    localStorage.setItem(key, JSON.stringify(TEST_VALUES.stored_value));
  ...
  it('Updates value and saves to localStorage', () => {
    const { result } = renderHook(() =>
      useVariableLocalStorage(key, TEST_VALUES.initial)
    );
    ...
    expect(localStorage.getItem(key)).toBe(JSON.stringify(TEST_VALUES.updated));
  });
